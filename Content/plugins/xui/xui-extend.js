...
xui.extend({
    ...
    */
    ...
    list: function (options) {
        ...
        var divPullDown = '<div class="pullDown" name="pullDown"><span class="pullDownIcon_xia"></span><span class="pullDownLabel">下拉刷新</span></div>';
        var divPullUp = '<div class="pullUp" name="pullUp"><span class="pullUpIcon_shang"></span><span class="pullUpLabel">上拉加载</span></div>';
        var ul = '<ul ismove="false"></ul>';
        ...
        var myScroll = new iScroll(scrollid, {
            ...
            onRefresh: function () {
                if (pullDownEl.getAttribute("name").match('loading')) {
                    ...
                    pullUpEl.childNodes[0].className = "pullUpIcon_shang";
                }
            },
            onScrollMove: function () {
                ...
                if (ispulldown == "loading" || ispullup == "loading")
                    return;
                //超过80开始刷新
                if (this.y > 80 && !pullDownEl.getAttribute("name").match('flip')) {
                    pullDownEl.setAttribute("name", 'flip');
                    pullDownEl.querySelector('.pullDownLabel').innerHTML = '松开立即刷新';
                    pullDownEl.childNodes[0].className = "pullDownIcon_shang";
                } else if (this.y < 0 && this.y < (this.maxScrollY - 80) && !pullUpEl.getAttribute("name").match('flip')) {
                    pullUpEl.setAttribute("name", 'flip');
                    pullUpEl.querySelector('.pullUpLabel').innerHTML = "松开立即加载";
                    pullUpEl.childNodes[0].className = "pullUpIcon_xia";
                }
            },
            onScrollEnd: function () {
                ...
                if (pullDownEl.getAttribute("name").match('flip')) {
                    ...
                    pullDownEl.childNodes[0].className = "pullDownIcon_loading";
                    if (typeof (pullDownAction) == 'function') {
                        pullDownAction(myScroll, el);
                    }
                }
                else if (pullUpEl.getAttribute("name").match('flip')) {
                    pullUpEl.setAttribute("name", 'loading');
                    pullUpEl.querySelector('.pullUpLabel').innerHTML = '正在加载...';
                    pullUpEl.childNodes[0].className = "pullUpIcon_loading";
                    if (typeof (pullUpAction) == 'function') {
                        pullUpAction(myScroll, el);
                    }
                }
    ...
    showloading: function (type) {
        ...
        var html = '<div id="loading_page" class="page" style="' + pageStyle + '"><div class="' + loadClass + '"></div></div>';
        this.top(html);
        return this;
    },
